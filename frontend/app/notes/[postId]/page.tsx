...
type DetailPost = NotePost & { attachment_urls?: string[] };
...
function getAttachmentKind(url: string): 'image' | 'pdf' | 'other' {
  ...
  if (lower.match(/\.(png|jpe?g|gif|webp|bmp|svg)(\?|#|$)/)) { // copilot generated the Regex
    return 'image';
  }

  if (lower.match(/\.pdf(\?|#|$)/)) { // copilot generated the Regex
    return 'pdf';
  }
...
function PostDetailPage() {
  ...
  const loadComments = useCallback(async (targetPostId: number) => {
    ...
    const { data, error: commentsFetchError } = await client
      .from('Comments')
    ...
    if (authorIds.length > 0) {
      const { data: authorRows, error: authorsError } = await client
        .from('Users')
  ...
  useEffect(() => {
    const loadPost = async () => {
      ...
      const mappedPost: DetailPost = {
        ...
        comments_count: 0,
        attachment_urls: row.attachment_url ? [row.attachment_url] : [],
      };

      setPost(mappedPost);
      setUserVote(null);

      const [{ data: voteRows, error: votesError }] = await Promise.all([
        client
          .from('Post_Votes')
          .select('value')
      ...
      if (!votesError && voteRows) {
        setUserVote((voteRows as { value: 1 | -1 }).value);
      }
  ...
  const handleVote = async (value: 1 | -1) => {
    ...
    setPost((current) => {
      ...
      return { ...current, votes: current.votes + delta };
    });
    ...
    try {
      if (existingVote === value) {
        ...
        await client.rpc('increment_votes', { post_id: post.id, delta: -value });
      } else if (existingVote !== null) {
        await client
          .from('Post_Votes')
          .update({ value })
        ...
        await client.rpc('increment_votes', { post_id: post.id, delta: value * 2 });
      } else {
  ...
  // same as notecard handle comment
  ...
  // copilot generated handDownloadAttachment code
  const handleDownloadAttachment = async () => {
    if (!currentAttachment || typeof window === 'undefined') {
      return;
    }
    ...
    try {
      ...
      link.href = objectUrl;
      link.download = getAttachmentFileName(currentAttachment);
  ...
  const attachmentUrls = post?.attachment_urls?.length
    ? post.attachment_urls
    : post?.attachment_url
      ? [post.attachment_url]
      : [];

  const currentAttachment = attachmentUrls[activeAttachmentIndex] ?? null;
  const currentAttachmentKind = currentAttachment ? getAttachmentKind(currentAttachment) : 'other';

  const reportQuery = post
    ? new URLSearchParams({
  ...
  if (!isMounted || loading) {
    return (
      // copilot made fun loading bubble from "make the loading post bigger with a rotating loading bubble thing"
      <main className="min-h-screen bg-zinc-50 px-4 py-8 text-zinc-600 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-5xl flex-col items-center justify-center rounded-2xl border border-zinc-200 bg-white p-12 shadow-sm sm:p-16">
          <div className="relative h-24 w-24">
            <div className="absolute inset-0 rounded-full border-2 border-zinc-200" />
            <div className="absolute inset-0 animate-spin">
              <div className="absolute left-1/2 top-1 h-5 w-5 -translate-x-1/2 rounded-full bg-[#7A1F1F] shadow-md" />
            </div>
          </div>
          <p className="mt-6 text-center text-3xl font-black tracking-tight text-zinc-800 sm:text-4xl">
            Loading post...
          </p>
          <p className="mt-2 text-sm font-medium text-zinc-500">Preparing the discussion thread and attachments.</p>
        </div>
      </main>
    );
  }
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <article className="rounded-3xl border border-zinc-200 bg-white shadow-sm">
          <div className="flex items-stretch">
            <aside className="flex w-17 shrink-0 flex-col items-center border-r border-zinc-200 px-2 py-5">
              <button
                type="button"
                onClick={() => void handleVote(1)}
                className={`rounded-full p-2 transition hover:bg-zinc-100 ${userVote === 1 ? 'text-orange-500' : 'text-zinc-500 hover:text-zinc-800'}`}
                aria-label="Upvote"
              ...
              <button
                type="button"
                onClick={() => void handleVote(-1)}
                className={`rounded-full p-2 transition hover:bg-zinc-100 ${userVote === -1 ? 'text-blue-500' : 'text-zinc-500 hover:text-zinc-800'}`}
                aria-label="Downvote"
                aria-pressed={userVote === -1}
              >
            ...
            <div className="p-5 sm:p-6 lg:p-8">
              <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">
                <span className="rounded-full bg-zinc-100 px-3 py-1 text-zinc-700">{post.course_label || 'Unknown class'}</span>
                <span className="rounded-full bg-zinc-100 px-3 py-1 text-zinc-700">{post.semester_label || 'Unknown semester'}</span>
              ...
              <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-zinc-500">
                    <span>
                      Posted by{' '}
                      <Link href={`/profile/${post.author_id}`} className="font-semibold text-zinc-800 underline-offset-4 transition hover:text-zinc-950 hover:underline">
              ...
              <div className="mt-6 whitespace-pre-wrap text-[1.05rem] leading-8 text-zinc-800">
                {post.body}
              </div>
              ...
              {attachmentUrls.length > 0 && (
                <section className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    ...
                    {attachmentUrls.length > 1 && (
                      <div className="flex items-center gap-2 text-sm text-zinc-500">
                        <button
                          type="button"
                          onClick={() => setActiveAttachmentIndex((current) => (current - 1 + attachmentUrls.length) % attachmentUrls.length)}
                          className="rounded-full border border-zinc-200 bg-white p-2 transition hover:bg-zinc-100"
                          aria-label="Previous attachment"
                        >
                          <ChevronLeft className="h-4 w-4" />
                        </button>
                        <span>{activeAttachmentIndex + 1} / {attachmentUrls.length}</span>
                        <button
                          type="button"
                          onClick={() => setActiveAttachmentIndex((current) => (current + 1) % attachmentUrls.length)}
                          className="rounded-full border border-zinc-200 bg-white p-2 transition hover:bg-zinc-100"
                  ...
                  <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white">
                    {currentAttachmentKind === 'image' ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={currentAttachment ?? ''}
                        alt={currentAttachment ? getAttachmentFileName(currentAttachment) : 'Attachment'}
                        className="w-full object-contain bg-zinc-950/5"
                        style={{ maxHeight: '36rem' }}
                      />
                    ) : currentAttachmentKind === 'pdf' ? (
                      <iframe
                        title={currentAttachment ? getAttachmentFileName(currentAttachment) : 'Attachment'}
                        src={currentAttachment ?? ''}
                        className="w-full border-0"
                        style={{ height: '36rem' }}
                      />
                    ) : currentAttachment ? (
                      <div className="flex flex-col items-center justify-center gap-3 px-6 py-10 text-center text-zinc-600" style={{ minHeight: '18rem' }}>
                        <Paperclip className="h-10 w-10 text-zinc-400" />
                        <p className="text-sm font-medium">Preview not available for this file type.</p>
                        <p className="max-w-xl text-sm text-zinc-500">{getAttachmentFileName(currentAttachment)}</p>
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {currentAttachment && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-100 hover:text-zinc-900"
                      >
                        <Paperclip className="h-4 w-4" />
                        Open file
                      </a>
                  ...
                  {attachmentUrls.length > 1 && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {attachmentUrls.map((url, index) => {
                        const isActive = index === activeAttachmentIndex;
                        ...
                        return (
                          <button
                            ...
                            className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-sm font-medium transition ${isActive ? 'border-red-200 bg-red-50 text-red-800' : 'border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-100'}`}
                          >
                              <span className="truncate" style={{ maxWidth: '14rem' }}>{getAttachmentFileName(url)}</span>
                          </button>
                        );
                      })}
                    </div>
        ...
        <section className="mt-6 rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-4 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <p className="text-xs text-zinc-400">Comments are visible on the post page.</p>
              <button
                type="button"
